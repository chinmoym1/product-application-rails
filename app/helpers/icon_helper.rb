module IconHelper
  def icon(name, custom_classes = "")
    # Default Tailwind classes for all icons
    base_classes = "w-5 h-5 #{custom_classes}"

    svg = case name.to_sym
          when :eye, :view
            <<-SVG
              <svg class="#{base_classes}" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
